...
from model.ABINet.resnet import resnet45,resnetLR
...
class ResTranformer_LR(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.resnet = resnetLR()
    ...
    def forward(self, SR_feat):
        output  = self.resnet(SR_feat)
        feature = output['layer5']
        n, c, h, w = feature.shape
        feature = feature.view(n, c, -1).permute(2, 0, 1)
        feature = self.pos_encoder(feature) # add PE
