...
from model.parseq.parseq import init_weights



class LowRes_PARSeq(nn.Module):
    def __init__(self,config):
        super().__init__()
        self.tokenizer = get_parseq_tokenize()

        img_size= [config.img_size[0]//2,config.img_size[1]//2]
        patch_size= [config.patch_size[0]//2,config.patch_size[1]//2]
        ...
        if config.full_ckpt is not None:
            logging.info(f'Read full ckpt parseq (stu) model from {config.full_ckpt}.')
            state = torch.load(config.full_ckpt)
            LR_state_dict = self.state_dict()
            pretrained_state_dict = {k: v for k, v in state.items() if 'patch_embed.proj.weight' not in k}
            LR_state_dict.update(pretrained_state_dict)
            self.load_state_dict(LR_state_dict)
    ...
    def training_step(self, images,parseq_info):
        ...
        output_dict['visual_feat']=memory # N,128,384
        tgt=parseq_info['target']
        tgt_perms=parseq_info['tgt_perms'][0] # multi-gpu trick
        tgt_in = tgt[:, :-1]
        ...
        for i, perm in enumerate(tgt_perms):
            ...
            output_dict['semantic_feat'].append(out)#[N,len(11??),C]
...
def parse_info_generator(tokenizer,label_str,device):
    """
    :return:
    """
    max_len = 26
    targets = tokenizer.encode(label_str, device)[:,:max_len+1]
    # ===============generate permutations for both tea and stu===========
    perm_forward = True
    perm_mirrored = True
    perm_num = 6
    max_gen_perms = perm_num // 2 if perm_mirrored else perm_num
    rng = np.random.default_rng()

    max_num_chars = targets.shape[1] - 2
    if max_num_chars == 1:
        return torch.arange(3, device=device).unsqueeze(0)
    perms = [torch.arange(max_num_chars, device=device)] if perm_forward else []
    max_perms = math.factorial(max_num_chars)
    if perm_mirrored:
        max_perms //= 2
    num_gen_perms = min(max_gen_perms, max_perms)
    if max_num_chars < 5:
        if max_num_chars == 4 and perm_mirrored:
            selector = [0, 3, 4, 6, 9, 10, 12, 16, 17, 18, 19, 21]
        else:
            selector = list(range(max_perms))
        perm_pool = torch.as_tensor(list(permutations(range(max_num_chars), max_num_chars)), device=device)[
            selector]
        if perm_forward:
            perm_pool = perm_pool[1:]
        perms = torch.stack(perms)
        if len(perm_pool):
            i = rng.choice(len(perm_pool), size=num_gen_perms - len(perms), replace=False)
    ...
    return {'tgt_perms':perms.repeat(len(label_str),1,1),'target':targets}
