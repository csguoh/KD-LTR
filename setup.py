...
class CharsetMapper(object):
    ...
    def logits_to_string(self,output):
        ...
        for o in out:
            text = self.get_text(o.argmax(dim=1), padding=False, trim=False)
            text = text.split(self.null_char)[0]  # end at end-token
            pt_text.append(text)
    ...
    def label_postprocessing(self, text):
        # transform the str lable into on-hot vector
        length = torch.tensor(len(text) + 1).to(dtype=torch.long)  # one for end token
        label = self.get_labels(text)
